...
public class IndividualLoanLogic 
{
	WebDriver driver;
	WebDriverWait wait;
	...
	public void Apply_Individual() throws Exception
	{
		...
		Thread.sleep(7000);
		String amountText = LoanAmount.getText();
		...
		Thread.sleep(7000);
		String tenureText = tenure.getText();
		tenureText.replaceAll("[^\\d.]", " ");
		...
		double loanAmount = Double.parseDouble(amountText); //144 -> Loan Amount    
		double annualROI = Double.parseDouble(interestText); //153 ->  Interest Rate            
		int tenureMonths = Integer.parseInt(tenureText); //161 -> Tenure
	    ...
	    System.out.printf("Flat ROI EMI: %.2f%n", flatEMI);
		 
		WebElement SystemEMI = driver.findElement(By.xpath("//span[@id='emi-value']"));
		String SystemsEMI = SystemEMI.getText();
		SystemsEMI.replaceAll("[^\\d.]"," ");
		Double SystemGeneratedEMI = Double.parseDouble(SystemsEMI);
		System.out.println(flatEMI);
		System.out.println(SystemGeneratedEMI);
		
		if(SystemGeneratedEMI == flatEMI)
		{
	...
	public static void main(String[] args) throws Exception {
		IndividualLoanLogic obj = new IndividualLoanLogic();
...
//		 double LocalCalulatedEMI = numerator / denominator;
//		 System.out.printf("%.2f%n", LocalCalulatedEMI);
//		 
//		 //for Flat ROI 
//		 double tenureYears = tenureMonths / 12.0;
//		 double totalInterest = (loanAmount * annualROI * tenureYears) / 100;
...
//		 double numerator = loanAmount * monthlyRate * Math.pow(1 + monthlyRate, tenureMonths);
//		 double denominator = Math.pow(1 + monthlyRate, tenureMonths) - 1;
//		 double ReducingEMI = numerator / denominator;  // reuse flatEMI variable to store reducing EMI
//
