...
public class Create_GoldLoan_ReducingROI extends BaseLoginForAllBranchModules
{
	WebDriverWait wait;
	...
	public void createGoldLoan_FlatROI() throws Exception
	{
        ...
        System.out.println("System Generated EMI For Reducing ROI = "+ShowEMIText);
        
        //Calculations for Reducing ROI
        ...
        double monthlyROI = loanROI / 12.0 / 100;
		double numerator = LoanAmount * monthlyROI * Math.pow(1 + monthlyROI, LoanTenure);
		double denominator = Math.pow(1 + monthlyROI, LoanTenure) - 1;
		double reducingEMIForGoldLoan = numerator / denominator;
		System.out.printf("Reducing GOLD LOAN ROI EMI: %.2f%n", reducingEMIForGoldLoan);
	    
	    if(Math.abs(systememiValue - reducingEMIForGoldLoan) < 0.01)
	    {
	    	//Submit
